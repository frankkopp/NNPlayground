...
class LinearClassifierTest {
  ...
  void score() {
    ...
    INDArray x = Nd4j.create(new float[] {-15f, 22f, -44f, 56f}, new int[] {4, 1});

    lc.setWeightsMatrix(weightsMatrix);
    lc.setBiasMatrix(b);

    INDArray scoreVector = lc.score(x);
    LOG.info("scoreVector P {}", scoreVector);
  ...
  void softmax() {
    ...
    double loss = lc.softmaxLoss(p, 0);
    LOG.info("Loss {}", loss);
    ...
    INDArray p2 = lc.softmaxScore(scoreVector);

    double loss2 = lc.softmaxLoss(p2, 2);
    LOG.info("Loss {}", loss2);
  ...
  void lossSVM() {
    ...
    double loss = lc.lossSVM(scoreVector, 2);
    LOG.info("Loss {}", loss);
